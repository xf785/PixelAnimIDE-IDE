# 瓦片地图模式（第 5 模式）设计文档

> 状态：算法层、工作流、UI（第 5 模式）均已完成并有测试保障；
> 第六轮按用户参考图 `示意图.png` 修正了拼装映射与提示词风格（见 §9~§10）。
> 相关模块：`core/tilemap/`（prompts / tiles / seamless / autotile / buildings / map）、
> `core/workflow/tilemap_workflow.py`、`ui/pages/tilemap_page.py`、`ui/widgets/tilemap_view.py`；
> 测试：`tests/test_tilemap_*.py`（14+27+6+6+12+14 = 79 项，含无缝不变量与 47 计数证明），全量 468 项通过。

## 1. 模式总览

1. **文生瓦片集**：文本描述纹理/风格 → 内置严格 3×3 九宫格提示词（中心=无缝
   纹理、四边=单侧直线边界的墙面瓦片、四角=圆角切角瓦片、纯白背景、统一
   调色板）→ 生图 API。
2. **自适应裁切**：`crop_base_3x3`——单格尺寸 = floor(min(w,h)/3) 取偶，
   整图居中取 3×cell 区域等分裁 9 张；`normalize_tileset` 归一化到目标规格
   （最近邻保持像素硬边）。
3. **逐瓦片重绘**（UI 待做）：复用 `core/editing/canvas.PixelCanvas`。
4. **无缝化算法**（`seamless.py`，确定性、无 AI）：
   - 纹理：顺序镜像平均（先水平后垂直，数学上保证 left==right 且
     top==bottom 逐像素相等），再重量化回原调色板；
   - 墙面：旋转到「边界朝上」，沿边界轴镜像平均（线带不动），四边线带
     统一为同一线色 → 长墙无限重复、四朝向风格一致；
   - 转角：由无缝中心纹理 + 统一线色按位掩码构图推导（构造性零接缝），
     AI 角瓦片仅可选低权重混合内部细节。
5. **47-tile / 双网格**（`autotile.py`，用户可选）：
   - 47 集：8 邻域位掩码（TL=1,T=2,TR=4,L=8,R=16,BL=32,B=64,BR=128）→
     几何构图：空侧画边界线；四角「两侧同空（外角）或两侧同满且对角空
     （内角）」时切半径 s/2 四分之一圆盘（弧线过边中点），弧线描边、弦
     不描边；地形一律取中心纹理。
   - 双网格：同一构图规则在 2 倍分辨率下以 4 类四分之一块（填充/上直切/
     左直切/转角盘）铺出，导出 2× 索引图。
6. **大网格预览**（UI 待做）：`TileMapModel` 铺设/擦除/矩形填充，按邻域
   位掩码自动衔接，`render()` 输出整图（47 或 dual 模式）。

## 2. 47-tile 数学依据

- 全 256 种掩码按视觉等价归并：k=4 全边满 16 种（四内角各由对角位决定）、
  k=3 缺一边 16、k=2 相邻 8、k=2 相对 2、k=1 仅一边 4、k=0 孤立 1，
  **总计 47**。实现为按组枚举 + 图像去重（`_group_masks`）。
- 孤立足（mask=0）与「四内角」（四边满、对角全空）图形重合（同为四角
  圆盘切），经典模板惯例保留两个槽位：实现输出 47 槽、46 张独立图像，
  `meta.mask_to_index` 覆盖全部 256 掩码。
- 无缝性不变量（有测试证明）：随机地图中任意相邻两格共享边上的不透明
  像素**逐像素一致**，唯一允许的差异位于共享边两端 r+2 像素内的转角区
  （内角缺口之「弦」落在相邻瓦片一侧，为 Godot 同款预期外观）。

## 3. 测试保障（tests/test_tilemap_core.py）

- 裁切自适应与规格归一；提示词强约束项。
- 纹理 wrap 逐像素相等（双轴）、确定性；墙面轴向 wrap + 线带统一；
  `process_base_set` 四边线色统一、转角与中心/墙面共享边一致。
- 构图几何：全邻=中心纹理；孤立四角切中心留；外角/内角/对角位语义。
- 47 集：计数、256 掩码全映射、46 唯一图 + 47 槽、全邻/孤立槽位校验、
  确定性。
- 地图：铺设/擦除/序列化、掩码值、随机图共享边不变量、双网格块类型。

## 4. 完成情况

- `core/workflow/tilemap_workflow.py`：步骤化工作流（prompts→base→crop→seamless→
  atlas→export），自动/手动双模式，编辑瓦片后可在无 API 下本地重跑后三步；
  `ui/workers.py` 增加 `TilemapWorker`（仅需图片 API）。
- UI：`ui/pages/tilemap_page.py`（参数表单 + 瓦片集预览 + 生成/编辑/地图预览）、
  `ui/widgets/tile_editor.py`（复用 PixelEditorWidget 逐瓦片重绘）、
  `ui/widgets/tilemap_view.py`（大网格预览：左键铺设/右键擦除/滚轮缩放/网格线）。
- 主窗口注册第 5 模式（侧栏第三行整行按钮 + 页面堆栈 index 4），i18n 中英文案。
- 测试：`tests/test_tilemap_workflow.py`（mock 端到端 47/dual、手动步骤、
  编辑后本地重跑、步骤守卫）、`tests/test_tilemap_gui.py`（模式切换、页面预览、
  瓦片编辑器、地图控件绘制/缩放、工作线程）；全量 411 项测试通过；
  打包验证：PyInstaller 重建后 exe 离屏启动正常。

## 5. 第二轮重构（按用户反馈）

1. **分类生成**：瓦片按功能分为两类：
   - 地块类 = 2×2 生态图（每块 3×3）：左上=纯基础地形（中心无缝纹理 +
     与空白的自然边界）；右上/左下/右下=同一生态下的特征地块（水塘/
     稀疏草地/岩石），特征块中心=特征纯纹理、周边 8 格=特征↔基础的
     直线/圆角过渡 → 同一生态内不同元素无缝融合；
   - 建筑类 = 2×2 建筑图：墙体组（中心=可重复墙面纹理 + 端头/转角）、
     顶面组、开口组（门/窗）、立柱组；建筑外纯白 → 抠除后透明，
     可像图层一样叠放在地块上。
2. **纹理退化修复**：无缝化从全图镜像平均改为**偏移错位缝合**（平移半格、
   仅中央十字缝窄带交叉淡化、平移回来）——接缝窄带外的像素原样保留，
   不再出现「渐变+色块」；单轴版 `make_axis_seamless` 用于边瓦片。
3. **47-tile 构图改用真实艺术片**：`compose_art_tile` 把每张瓦片的 4 个
   四分之一块直接取自九宫格的对应艺术片（中心=填充、边=直线过渡、
   角=圆角过渡，内角=角艺术），块接触带 1px 交叉淡化掩盖 AI 不一致；
   不再程序化描边。`build_47_sheet_art` / `build_dual_pieces_sheet_art`。
4. **多地形 + overlay 地图模型**：`TileMapModel` 支持 terrain id 网格 +
   `terrain_sets`（每地形一套九宫格）；`mask_for_terrain`：基础地形把
   任何非空地形视为满（内部过渡显示填充），特征地形仅统计同地形邻居
   （过渡艺术由特征一侧提供）；`overlay` 层放建筑拼件（直段/端头/转角/
   立柱，带旋转），`process_building_sheet` 抠白底 + 派生拼件。
5. 测试：`tests/test_tilemap_eco.py`（缝合保纹理/生态裁切/艺术构图映射/
   47 计数/多地形掩码/overlay）。
6. UI 与工作流已接入：`tilemap_workflow.py` 按 category 分支（ground=生态
   多地形 47 艺术集导出；building=拼件导出 + 摆样预览；classic=旧链路）；
   `tilemap_page.py` 类别选择 + 特征槽 + 按瓦片组编辑 + 多地形画笔；
   `tilemap_view.py` 地形画笔切换与建筑拼件放置（选择 + 旋转 + 图层）。
   待办：实机验收（真实生图服务商 2×2 生态图/建筑图的裁切对齐与融合效果）。

## 7. 第三轮修正（按用户反馈）

- **两段式确认**：生态/建筑生成改为「底图生成 → 保留中间结果并展示 →
  用户点「接受并生成瓦片集」（或「重新生成」）→ 本地继续裁切/无缝/瓦片集/
  导出」；`run_to_base` / `finish_from_base`，底图始终存盘 artifacts。
- **地块类必须填充满**：经典链路与演示地图全部改用九宫格艺术片构图
  （`compose_art_tile` + `prepare_terrain_set`），瓦片全程不透明、无程序化
  描边与透明楔形；演示地图整图铺满（无空边）；47 集图仅第 48 空槽透明；
  经典 3×3 提示词同步改为「角格 = 画在格内的圆角过渡、整格全填充」。
- 新增测试：`test_two_stage_base_then_accept`、`test_classic_map_preview_
  fully_opaque`、GUI 两段式接受流程测试；全量测试通过。

## 8. 第四轮：参照公开 blob autotiling 参考实现修正（关键 bug）

参考对象：`frameronin.com` 为 SPA（无源码可爬），改用其开源仓库
`systemchester/FrameRonin` 的实现（本地 `.tmp/refs/FrameRonin`），
核心参考 `frontend/src/components/infiniteMap/blobTerrain.ts`：
`computeBlobMask`（掩码约定）、`MASK_TO_INDEX`（47 槽索引表）、
`nearestBlobTileIndex`（汉明距离回退）。

学到的两条关键规则（此前实现缺失，正是「拼接很烂」的根因）：
1. **对角位必须依赖正交位**：`n && w && has(-1,-1)` 才计入 TL 位（TR/BL/BR 同理）。
   此前我们无条件计入对角位 → 产生大量非经典邻域（如「只有对角邻居」），
   派生出错误瓦片；改为 `canonical_mask()` 规范化后，**可达掩码恰好 47 个**，
   与 47 槽一一对应。
2. **任意掩码用汉明距离回退到最近合法掩码**（`nearest_mask`），导出的
   `mask_to_index` 因此对全部 256 种邻域都有确定答案。

本轮修复的 bug：
- `mask_from_neighbors` / `mask_for_terrain` 未规范化对角位（见上，核心 bug）；
- `build_47_sheet_art` 改为枚举 47 个可达掩码 + 全 256 映射 + `reachable_masks`
  元数据（槽位稳定，不再依赖去重顺序）；
- `compose_art_tile` 接触带融合**写回顺序 bug**：第二次写回读了已被改写的
  邻居列/行，导致中线左右不对称的脏边（已用副本修复，并有对称性测试）；
- `TileMapModel.render()` 在非多地形模型下**丢弃 overlay**→ 建筑地图预览全透明
  （严重 bug，已修）；`center=None` 时也可渲染（仅 overlay）；
- overlay 未序列化 → 重开「地图预览」拼件全部丢失；现按名称序列化 +
  `from_dict(pieces=...)` 恢复，建筑导出补写 `map_demo.json`；
- 地图预览对话框在建筑类别下 `session.processed` 为 None 导致崩溃/空图（已修）。

新增测试：`test_canonical_mask_rule`、`test_nearest_mask_fallback_and_full_mapping`、
`test_compose_art_blend_is_symmetric`、`test_overlay_serialization_and_overlay_only_render`、
建筑预览非空回归；全量测试通过。

## 9. 第五轮：2×2×3 地形生成链路严格修正

审查发现三处**致命错误**（已复现证据），是「生成效果非常不好」的直接原因：

1. **裁切窗口 bug（最严重）**：`crop_blocks(img, tile_size=32)` 用
   `cell = min(tile_size, w//6, h//6)` 把「目标瓦片尺寸」当成了裁切窗口，
   对 768×768 的 6×6 生态图**只取了中心 192×192 的一小块**，AI 画的网格被
   整体切错位（复现：右下角格标记完全取不到）。
   修正：`crop_blocks` / `crop_base_3x3` **始终按整图比例裁切**
   （cell = min(w//cols, h//rows)），目标尺寸只由 `normalize_tileset` 归一。
2. **提示词与请求尺寸矛盾**：提示词写「每格 32×32 像素」，而实际请求
   768×768（每格本应 128px）→ 模型按文字画出小网格并在四周留白，进一步
   加剧错位。修正：新增 `grid_cell_px()` 统一计算
   「请求边长 = 格数 × 单格像素（32/64 的倍数，≤2048）」，并把同一数值写进
   提示词（生态/建筑 6 格、经典 3 格），工作流日志同步打印。
3. **几何契约缺失**：原提示词只说「smooth transitions」，AI 可能把边界画在
   1/3 处或让特征填满整格，而我们的四分之一块拼接**要求边格严格沿中线对半、
   角格外侧 1/4 为基础**。修正：提示词改为逐格几何契约（top/bottom/left/right
   边格的哪一半是基础地形、角格 1/4 + 圆角弧、基础块 9 格同一无缝纹理且
   跨块像素级一致），并显式禁止透明度/空区/装饰物。

附带质量改进：
- `normalize_tileset` 缩小改用**块众数降采样**（全向量化、确定性），
  4 倍放大的像素画可被精确还原，不再点采样丢格色；
- 建筑提示词同步改为同一几何口径（6×6 格、边长=$cell×6、墙体中心格必须
  填满整格、其余格纯白待抠除）。

新增测试：`test_crop_uses_whole_sheet_not_center_window`（整图裁切回归）、
`test_grid_cell_px_is_multiple_and_within_limit`、`test_mode_downscale_recovers_pixel_art`、
`test_ground_request_size_matches_prompt`、`test_classic_request_size_matches_cells`、
`test_building_request_size_matches_cells`、提示词几何契约断言；全量测试通过。

## 10. 第六轮：按「示意图.png」参考修正拼装映射与风格

用户提供参考图（`示意图.png`，526×528）作为期望效果。对其做定量分析
（按 6×6 格切分统计每格亮度/深色占比 + 局部放大目视）后确认：

- 每块 3×3 是**一个带深色描边的圆角团块**（水塘/岩块），团块边界条带位于
  边格的**外侧约 1/4**（不是经典模板的 1/2）；角格外侧为基础地形 + 圆弧；
- 基础地形块中心格为纯净无缝纹理；
- 特征具备明显深色描边 + 2~3 阶明暗 + 内部纹理（水波、岩石切面）。

据此修正两处**生成逻辑核心问题**：

1. **拼装取块：从「下半/右半四分之一」改为「同侧四分之一」**（`_ART_SRC`）。
   旧实现假设 AI 把边界画在**中线**（取源瓦片下半/右半），而参考图画在**外侧 1/4**，
   于是条带被挪到目标瓦片中线甚至整条丢失 → 相邻瓦片交界纹理/描边错位
   （用户反馈「拼接很烂」的核心原因之一）。
   新实现取 `top.TL / left.TL / bottom.BL / right.BR` 等同侧四分之一，
   对 AI 画在 1/4 或 1/2 处**都能正确对位**（1/2 时两种取法等价）。
2. **内角改用对角瓦片旋转 180° 的负形**。旧实现外角/内角不分，内角也用本角凸角
   艺术 → 内角处出现错误圆角/缺口；现在内角（对角为另一种地形、两侧同类）取
   `rot180(对角瓦片)` 的同侧四分之一，得到正确凹角。

提示词同步改为参考图口径：特征块 = 「一整块圆角团块 + 外侧约 1/4 条带
（1/5~1/3 且各格一致）+ 深色 1-2px 描边 + 2-3 阶明暗 + 内部纹理」；
删除旧提示词中会把参考风格抹平的 `no shading / no outlines` 约束；
基础块中心格要求纯净无缝。

验证：
- 新增 `test_art_strip_keeps_ai_boundary_depth`（合成 1/4 条带底图，断言拼装后条带
  仍在 1/4 处、外角/内角形态正确；旧代码必失败）、更新
  `test_compose_art_edge_mapping`（外角=本角瓦片、内角=对角瓦片）；
- 端到端目视验证：按参考风格合成 6×6 底图 → 走真实管线（裁切→预处理→多地形渲染），
  边界列采样为 `base/base/…/rim/feature`（外侧 1/4 条带 + 深色描边逐像素正确），
  外角圆角、内角凹角形态正确，无透明空洞；
- 全量测试通过（当时 452 项；第八轮后为 468 项）。

附带：`conftest.py` 增加受限环境兼容（`tmp_path` 用普通 mkdir 建在 `.tmp/pytest_work`，
规避 Windows 沙箱下 pytest 编号目录 chmod/加锁/枚举被拒的问题）。

## 11. 第七轮：基础地形块位置自动识别（防「整张图角色错位」）

### 问题
裁切后的角色分配原来是**纯位置约定**：`blocks[0][0]` 恒为基础地形，其余三块按
右上/左下/右下对应特征。但生图模型经常不守位置要求——真实例子是用户的
`示意图.png`：纯基础地形块在**右上**，冰水塘在左上。此时按位置约定会把「水塘团块」
当成地面纹理、把纯地面当成一个「特征地形」，整张瓦片集彻底不可用（且因为裁剪
无误、流程不报错，属于最难排查的一类失效）。

### 判据（几何契约的逆命题）
特征块的结构是「中心格=纯特征，外侧约 1/4 条带=基础地形」，因此**外侧带颜色与
中心格颜色必然不同**；基础地形块则处处同一纹理。`block_base_score()` 打分：

1. `ring_gap`（主项）：外侧带像素到**中心格中位色**距离的**中位数**——中位数对邻块
   溢出的碎石/装饰点稳健（参考图右上块的外侧带就混入了水塘的岩边碎点）；
2. `centre_spread`：中心格自身的杂色度（特征纹理比纯地面花）；
3. `block_std`：整块亮度标准差——防止「团块画得太小、中心格仍以基础色为主」时被
   误判成地面（此时唯一暴露它的是整块的强对比）。

`detect_base_block()` 取得分最低者并给出可信位：
- 四块得分都高（AI 压根没画纯基础块）→ 回退左上并标不可信；
- 最优与次优几乎同分（例如两块都是同一地面）→ 仍取最低分，但标不可信。

### 接口与交互
- `ecosystem_from_blocks(..., base_pos="auto"|"tl"|"tr"|"bl"|"br")`；`EcosystemSheet`
  新增 `base_pos` / `base_pos_detected`；
- 工作流参数 `TilemapParams.base_block`（默认 `auto`），裁切日志打印
  「基础地形块位置：右上（自动识别）」；识别失败给 warning 建议重新生成或手动指定；
- UI：参数区新增「基础地形块」下拉（自动识别/左上/右上/左下/右下，仅地块生态类别
  可见）。**该项在点「接受并生成瓦片集」时才生效**——用户对着底图即可纠正误判，
  裁切/无缝/瓦片集全是本地步骤，无需重新生图；
- 提示词补一条强约束：基础块是**唯一**没有任何边界/描边/碎石/贴花的块，工具靠
  「纯净度」自动定位它，不纯净则整张瓦片集不可用。

### 验证
- `test_detect_base_block_finds_plain_block_in_any_position`（参数化四个位置，纯块
  得分 <6、其余 >60）、`test_ecosystem_blocks_use_detected_base_and_reading_order`
  （识别 + 其余三块按阅读顺序对特征名）、`test_base_pos_explicit_override_wins`、
  `test_detect_base_block_falls_back_when_no_plain_block`（四块全是团块 → 回退左上
  且不可信）、`test_plain_sheet_detection_is_safe`（只画了地面）；
- 工作流级：`test_ground_workflow_detects_base_block_position`（底图纯地面块在右上
  → 自动识别、日志留痕、基础地形确实是那块纯纹理）、`test_ground_workflow_manual_base_block`；
- GUI 级：确认流程里改选「右下块」后 `ecosystem.base_pos == "br"`；
- 端到端目视：按参考图口径合成 6×6 底图（基础块在**右上**、团块带 1/4 基础条带 +
  岩边 + 圆弧圆角）→ 真实管线 → 铺设含**内凹角**（水域中间挖空）的地图，
  外角圆角、内角凹弧、条带深度与岩边逐像素与底图一致。

### 建筑类底图的自检（同一类失效的兜底）
建筑四块的语义（墙体/顶面/开口/立柱）在像素上没有可靠的可判别特征，仍按位置约定，
但新增廉价的**自检**：墙体组（左上块）中心格必须填满整格，`opaque_ratio()` 低于
90% 即在日志给出 warning 并提示「墙体组画在左上、立柱组画在右下」——
覆盖「AI 把白底拼件块画到左上」这类会静默产出坏拼件的情况。
测试：`test_building_sheet_self_check_warns_on_white_wall_block`。

## 12. 第八轮：格线框清理 / 彻底禁文字 / 47 拼接算法重写（对齐式构图）

用户反馈（真实生图底图）：① 底图每格有明显**深色线框**；② 底图上偶尔出现
「草地」这类**文字**；③ 由底图推导的 47 瓦片**衔接明显有问题**。

### 12.1 根因诊断（先复现，再改）
用「参考图口径 + 每格 2px 深色线框」合成底图跑一遍旧管线，成品地图上出现**规律性
深色网格**——线框被四分之一块抠图当成艺术内容拼进了每一张瓦片。此外旧实现还有
两个结构性缺陷：
- 条带/描边等「另一方地形」的像素来自 **AI 的边格**，与基础地形瓦片的纹理**相位不同**
  → 交界处颜色/图案跳变；
- 逐块抠图依赖 AI 把边界画在格内哪个位置、九格是否连续，AI 一换画法就错位。

### 12.2 清理流程（新增 `core/tilemap/cleanup.py`）
- `strip_grid_frames()`：在候选格线位置比较「线带 vs 两侧参考带」亮度中位数，
  显著更暗才判定为线框（参考亮度取**较暗一侧**，避免深色地形贴着线时判成更宽的线）；
  用带外最近两像素的**下中位数**填回（取真实像素、不做平均，不会在「地形色边界正好
  落在格线上」时留下中间色涂抹带）。图像外缘的线框也一并处理。
  清理后的底图另存 `artifacts/tileset_sheet_clean.png`，界面上确认底图时预览的就是它。
- 文字：`detect_text_marks()` 用**等宽细笔画签名**（笔画宽度众数占比 ≥0.35 且细笔画
  ≥6 条）+「多段细笔画/多个等高小块」判据；实测「草地/雪原/GRASS/water/12」全部命中，
  而岩石、草丛、菱形贴花、碎石、纹理噪点**零误报**。命中后 `patch_marks()` 用「自身纹理
  平移半格」的副本覆盖，并把修补写回清理后的底图（预览、产物与成品一致）。
  检测在**源分辨率**的原始格上做（归一化后笔画会被压碎），四块中心格都查。
- 纹理兜底：基础地形的纹理由**九格逐像素中位数**（`median_tile_texture`）得到——
  只画在个别格里的文字/签名/贴花会被「投票」掉。

### 12.3 47 拼接重写：对齐式构图（AI 纹理 + 程序化几何）
`compose_art_tile()` 不再逐块抠 AI 九宫格，而是只用 AI 的**纹理与实测参数**：

| 元素 | 来源 |
|---|---|
| 特征纹理 | 该地形中心格 → 去外框 → 偏移缝合 → 缩放到瓦片尺寸 → **再缝合一 次**（缩放会破坏边缘相等性） |
| 另一方地形纹理 | 基础地形块的九格中位数纹理（所有地形共用同一份） |
| 条带厚度 band / 描边宽度 rim / 描边色 | `measure_terrain_art()` 从 AI 边格实测：从外向内第一个「像特征色」的像素深度（四侧三格取中位数）、条带内明显暗于地面的像素宽度与颜色 |
| 几何 | 暴露侧画 band 宽的地面条带；转角处内边界为半径 band 的圆弧（外角：把地形的角切掉、地面沿弧补齐，弧与两侧直边**相切**；内角：角上半径 band-1 的地面凹口，保证沿边长度正好 band）；描边 = 地面内侧的等宽带（**圆盘**膨胀，方形膨胀在对角方向会多出 √2 倍宽） |

**无缝由构造保证**：所有瓦片取同一份「网格对齐、左右/上下边缘逐像素相等」的纹理，
几何只依赖位掩码 → 相邻瓦片共享边像素相等（仅在转角 ±(band+2) 像素内，两条描边线
交汇处允许 1-2 像素差异）。这同时消除了「AI 画在 1/4 还是 1/2、格子是否连续、有没有
线框」的全部影响。基础地形块 `plain=True` 不实测（它本来没有边界），避免底图脏像素
污染地图外缘的边框线。

### 12.4 提示词
新增 `NO_TEXT_RULE`（三个类别的提示词全部追加）：显式禁止任何文字/字母/汉字/数字/
标题/标签/签名/logo/水印/箭头/UI，并要求**不得写材质名**；同时禁止格线/边框/表格线/
分隔框，强调「格子之间只能由画面本身区分，画面要连续跨过格线」；负向词表同步补齐
（frames、table lines、letters、font、caption、logo、signature、stamp…）。

### 12.5 验证
- 单元：`tests/test_tilemap_cleanup.py`（14 项）——线框抹除只改线框像素且平坦区与
  「本来无线框」的底图一致；文字 4 组正例全中、5 组自然物零误报；修补后文字色不再
  出现；九格中位数能抹掉单格文字；对齐式纹理边缘相等；随机多地形地图共享边在非转角带
  逐像素相等；条带/描边实测结果（1/4 + 1/16 → 32px 瓦片 band=10、rim=2）；
  工作流级（清理日志 + 清理后底图落盘 + 成品瓦片不再含线框颜色；文字告警 + 修补）。
- 端到端目视：合成「参考图口径 + 2px 格线框 + 基础块写字」的底图 → 真实工作流 →
  地图呈现圆角团块 + 连续描边 + 无网格、无文字（清理后的底图也已落盘核对）。
- `tests/test_tilemap_eco.py` 中原「按四分之一块抠图」的断言改写为对齐式构图的
  几何断言（全邻=纯纹理、暴露侧条带/描边位置、外角相切圆弧、内角凹口长度）。

## 14. 第十轮：建筑类链路重写为「墙体 16-tile 族 + 透明外部」

### 旧实现的问题
`process_building_sheet()` 只产出 4 个拼件：整格实心「直墙」、圆盘切两角的「端头」、
圆盘切一角的「转角」、以及 AI 立柱。于是：没有墙体厚度、没有 T 形/十字/内角/外角/
门/顶面，铺出来只能是方块堆叠；而且**没有接缝保证**（相连侧也会被当作暴露面画出
描边，长墙每格出现一道竖线）。

### 新设计（`core/tilemap/walls.py`）
1. **16-tile 族**：墙体是一维路径，一格只需要四个方向是否相连（`n|e|s|w` 四位），
   16 种组合正好覆盖 墙内部 / 墙边 / 墙角（内角与外角同时存在于一块里）/
   T 形（4 向）/ 十字 / 端头（4 向）/ 孤立块；对角位对墙体没有意义。
   槽位 = `n | e*2 | s*4 | w*8`，与地块类的 16 图块族约定一致。
2. **固定截面几何**（关键）：横带的行范围、竖带的列范围只由 `margin` 决定，
   相连只是把带子**伸到那条边**并在中心汇合 —— 因此 A 的 E 位与 B 的 W 位同时
   为满时，两块在共享边上的墙/透明分界完全一致；直墙、T 形、转角之间也不会错位
   （先前「让位深度随另一条轴的连通情况变化」的写法会导致 1 像素错位，已废弃）。
3. **透明外部**：让位区 alpha=0、无半透明、无白边（白底先经白键抠除，轮廓是程序化
   1px 描边，且只在**暴露**边界上画 —— 相连侧不画，长墙因此不会每格一道竖线）。
4. **层次**：朝上/朝左的边界是墙顶（AI 顶面组纹理优先，否则提亮），朝下/朝右的
   边界是立面（压暗），边界朝向由足迹几何算出，因此任何形状的外观规则都一致。
5. **转角**：弯折外角用「角部方形 − 切圆」倒圆（与两条墙面相切）、内角补小圆角；
   倒角可能切出的 1px 孤岛由「只保留最大连通块」清掉。
6. **门与立柱**：门件取自 AI 开口组中心格（贴在直墙上，另有竖直版），
   立柱件取自 AI 立柱组中心格，取不到就程序化生成柱体。
7. **实心件**：额外导出 `solid`（整格不透明）供整片实心区域使用；默认**不**自动
   替换 16 族槽位 —— 实心块与带子家族形状不同，混用会在共享边留下透明/不透明错位。

### 导出与接入
- `build_wall_atlas()`：4×5 = 20 槽图集（0..15 = 16 族，16 = solid，17/18 = 门横/竖，
  19 = 立柱），元数据含 `bits / slots / slot_of_piece / extra_slots /
  mask16_to_slot / mask8_to_slot（256 项）/ transparent_outside`；
- 工作流：`_do_seamless`（建筑）改为 `wall_art_from_sheet` + `build_piece_set` 并
  生成图集；`_do_atlas` 幂等补齐；`_export_building` 导出 `pieces/*.png`（20 张）、
  `walls_16.png/json`，并把演示地图改成「地块铺底 + 墙体透明叠加」的一间屋子
  （外墙 + 上下门洞 + 内部十字墙），直观展示 16 族全部连接件；
- 参数：`TilemapParams.wall_thickness`（墙体厚度占比，默认 0.56）与既有 `edge_noise`
  （墙体带子外侧的起伏，共享边仍逐像素一致）。

### 验证
`tests/test_tilemap_walls.py`（13 项）：16 族完整性与命名、非内部件透明（无半透明、
无近白像素）、实心/十字件的透明比例、带子恰好居中 `[margin, s-margin)`、转角无孤立
像素且为 L 形、**整张墙体地图（外墙 + 内十字）0 处共享边不一致**、长墙相连侧中段
无描边、AI 建筑图推导（纹理网格对齐 + 图集槽位/元数据）、旋转保持透明度。
同时更新了建筑端到端与 GUI 拼件下拉的断言；全量测试通过。










## 15. 第十一轮：瓦片包（保存/加载/叠加）、预览交互升级、柏林噪声大地图

- **瓦片包**（`core/tilemap/pack.py`）：把一次生成存成单个 `.tilepack`（zip = manifest.json +
  各地形特征纹理/另一方地形纹理 + 建筑墙纹理/顶面/门/立柱 + 整族拼件），保的是**纹理与实测
  参数**而不是只有图集，因此加载后能按掩码重新构图。地块包与建筑包可同时加载到一个预览里
  看综合效果。`pack_from_session()` 直接从会话建包；主页面与预览对话框都有「保存瓦片包」。
- **预览交互**（`ui/widgets/tilemap_view.py`）：画布放进滚动区，**Ctrl+左键拖动平移**、
  滚轮缩放、**网格显示/隐藏**开关（工具栏复选框）、**「添加瓦片包」**把一个包并入当前
  预览（地形分配空闲 id、拼件加包名前缀，工具栏自动重建）；绘制坐标改走 `mapFrom`，
  滚动后落笔不偏移。
- **柏林噪声大地图**（`core/tilemap/bigmap.py` + `core/tilemap/terrain_noise.py`）：`generate_perlin_map()` 用
  Perlin/FBM/河谷/山地场生成 24~400 格的地形网格并映射到水/平原/山地三类；`scatter_walls()`
  可在平原上自动散布小屋（16-tile 族自动选型 + 门洞），用于预览「地块 + 建筑」叠加。
  渲染新增 `compose_art_tile_cached()`（按掩码缓存合成结果），96×64 大地图秒级出图。
- 测试：`tests/test_tilemap_pack.py`（6 项）——地形/建筑包往返一致（纹理与 art_meta 全等、
  构图结果一致）、会话建包、坏文件报错、程序化地图确定性/地形覆盖、大地图渲染不透明、
  散布建筑写入 overlay 且只用 16 族件；全量测试通过。
## 16. 第十二轮：类别精简、基础地形无边、素材（道具）链路、墙体实时自动拼接

1. **移除「经典 3×3」类别**：界面只保留 地块生态 / 建筑类 / 素材（道具）；`classic`
   代码路径保留以兼容旧项目与测试（默认 category 仍为 classic，仅供 API 调用）。
2. **基础地形不再有边界艺术**：`align_terrain_set(plain=True)` 写入 `no_border`，
   构图时整格直接输出无缝纹理 —— 地图外缘（邻居为空）不再画条带/描边，
   消除预览图左下角那圈「深色硬边」（草地这类高饱和地形尤其明显）。
3. **素材（道具）生成链路**（`core/tilemap/props.py` + 工作流 `prop` 类别）：
   - 提示词：N 个变体排成网格、纯色底（白/洋红/绿，四边中位色自适应）、物件完整且底部居中、
     硬边像素风、严禁文字/格线/渐变；
   - 处理：**按四边中位色做边缘洪泛抠底**（只删与边缘连通的背景，物件内部同色区域不会被挖空）、
     alpha 严格 0/255（无灰边）、裁紧内容后等比缩放并**底部对齐**；
   - 产物：逐个 PNG + 打进 `.tilepack`（`props/<名字>.png`），地图预览里
     「添加瓦片包」即可当画笔铺；界面在素材模式下可多次生成不同素材，累积进同一个包后导出。
4. **建筑类实时自动无缝拼接**：`TileMapModel` 新增**墙体层**（`wall_grid` + `wall_pieces`），
   涂/擦墙时按四邻域实时求 16-tile 掩码并自动选件（转角/端头/T 形/十字全自动），
   透明外部叠在地块上；`to_dict/from_dict` 序列化墙体层，预览视图新增「自动墙」开关
   （左键涂墙、右键擦除，无需手工挑件）。
5. 测试：`tests/test_tilemap_props.py`（6 项：提示词网格与底色、边缘洪泛抠底不挖空内部、
   裁紧与底部居中、命名与空格子跳过、素材包往返、素材端到端）、
   `test_wall_layer_auto_stitches_in_realtime`（掩码自动选型 + 序列化一致）；全量测试通过。
## 17. 第十三轮：素材抠底加固、素材可叠加与缩放放置、拼件画笔随时可用

针对反馈的三点：

1. **素材背景没抠净 → 抠底加固**（`core/tilemap/props.py`）
   - 背景色改为取**四角小块**的中位色（整条边容易被压边的物件带偏）；
   - 容差**自动升级重试**（38 → 58 → 84 → 120）：AI 的「纯色底」常带轻微渐变/噪点，
     原来固定 38 会留下整块底色；每轮都从四边**洪泛**，只删与边缘连通的同色像素，
     因此物件内部的同色区域依旧不会被挖空；
   - 仍然不透明（物件与背景几乎同色）时做一次**全局同色删除兜底**，宁可输出干净剪影；
   - 抠底后仍占 >85% 格面积会写 warning 日志，便于定位是哪张底图有问题；
   - 页面在素材生成后会显示「已抠背景」的横排预览（棋盘格衬托透明区），一眼可确认。
2. **素材可叠加在地块上**：素材包（`.tilepack`，category=prop）加载进预览后即进入
   拼件画笔，左键铺在地块/建筑之上（overlay 层），与建筑拼件同一机制。
   顺带修掉一个真 bug：拼件下拉原先只在「构造时已有拼件」才创建，
   预览里**后加**的包用不了画笔 —— 现在画笔控件始终存在，按可用性启用/禁用。
3. **素材大小可调**：`TileMapModel.set_overlay(..., scale=…)` 支持放置缩放，
   渲染时以**格子底部中心**为锚点缩放（放大的树仍「站」在同一格地面上），
   并且 `scale` 会写进地图 JSON、恢复后渲染逐像素一致；预览工具栏新增「缩放 %」
   （25%–400%），与旋转、自动墙开关并列。

测试：新增 4 组抠底场景（纯白/洋红/噪点底/物件压边，断言不透明占比 < 0.6）、
背景与物件同色时的兜底、overlay 缩放跨格与序列化一致、后加瓦片包立即可用且缩放在放置时生效；
全量测试通过。
## 18. 第十四轮：地图预览免生成直接进入 + 包内直接铺设

- 「地图预览」按钮**不再依赖先生成**：页面初始化即可点击（带 tooltip 说明）；
  未生成时用 `scratch_map_model()` 建一张空白地图（尺寸/瓦片大小取当前参数），
  进入后用「添加瓦片包」加载地块 / 建筑 / 素材包即可直接铺设；
  没有会话时底栏按钮变成「关闭」（不做「应用到会话」），并给出提示文案。
- `TilemapView.add_pack()`：把地块包加进**空地图**时自动整图铺满该包的基础地形，
  并把地形画笔切到它 —— 打开预览→添加包→立刻能画，不用先手动画底。
- 测试：`test_map_preview_available_without_generating`（按钮可用、空白模型尺寸取自参数、
  加载包后自动铺满并可继续铺设）；全量测试通过。
## 19. 第十五轮：完整瓦片集导出/导入、交界渗透融合、素材白/黑底扣除、双网格

1. **导出改成「完整瓦片集目录/压缩包」**（不再是 `.tilepack` 本体）：
   `export_tileset_dir()` 输出 `manifest.json`（可再次导入）+ `README.txt` +
   `textures/`（地形特征纹理与另一方地形纹理）+ `atlas/`（47-tile 8×6 图集、
   建筑 16-tile 图集，各带 .json 掩码索引）+
   `tiles/terrain_<id>/`（**逐张单独瓦片**，文件名含槽位与掩码 + index.json）+
   `pieces/` 或 `props/` + `map/info.json`（全部元信息），并另存一份同名 `.zip`。
   工作流三个类别（地块/建筑/素材）都改为导出该目录；页面按钮改为「导出瓦片集」。
   **导入**统一走 `load_tileset()`：文件夹、导出 zip、旧 `.tilepack` 都能读。
   同时移除演示地图来源里的程序化地形选项。
2. **不同地形交界渗透融合**：新增 `edge_blend`（默认 0.5，界面「交界融合 %」），
   `compose_art_tile` 在边界两侧用确定性噪声互相「咬合」（地形吃进地面带、地面吃进
   地形侧），交界呈不规则渗透状而非一条硬边；噪声幅度在瓦片边缘衰减到 0，
   因此同类相邻瓦片的共享边仍逐像素一致（有测试保证）。
3. **素材白/黑底彻底扣除**：先从描述粗判主体是否可能含浅色（雪/云/白/骨/纸/冰…），
   不含浅色 → 提示词用**纯白底**并**直接删除所有通道 ≥214 的像素**；
   可能含浅色 → 用**纯黑底**并删除所有通道 ≤41 的像素；再叠加 flood-fill 兜底。
   测试断言抠底后"近白/近黑"不透明像素数为 0。
4. **双网格（dual grid）地块类智能支持**：`TileMapModel.dual_mode` 让地块预览按
   4 个四分之一块拼装（同一套对齐式艺术，`_quarter` 决定每块的切型），
   `atlas_mode="dual"` 时演示地图自动切到该模式，`to_dict/from_dict` 保留该开关。
   测试断言双网格渲染填充满、与 47 单格渲染不同、序列化可恢复。
5. 测试：新增 `tests/test_tilemap_round14.py`（8 项）；全量测试通过。
## 20. 第十六轮：素材导出 zip、跨包交界块状渗透融合、文档与截图画廊

1. **素材导出也是 zip**：素材（道具）走与地块/建筑相同的 `export_tileset_dir()`，
   输出「完整瓦片集目录 + 同名 zip」（逐张素材 PNG、index.json、map/info.json、
   manifest.json、README.txt）；页面按钮在素材模式下也改为目录+zip 导出。
2. **跨瓦片包交界的块状渗透融合**（`TileMapModel._blend_terrain_edges`）：
   同一包内部靠「对齐式构图 + 边缘噪声」保证无缝；但**两张来自不同瓦片包的地块**
   相接时，各自带的是自己的地面条带与描边，交界会是一条硬边。现在在合成后的画布上
   沿共享边做**块状噪声互换**：每 `block×block`（默认约 tile/12）一段随机决定互相
   咬进 1..depth 像素（depth ≈ 强度 × tile/6），两侧呈块状互相渗透；
   噪声只依赖格坐标 → 结果确定性可复现；融合强度沿用界面的「交界融合 %」，
   写进地图 JSON（`edge_blend`）随预览/导出保留。
3. **文档同步**：`README.md` / `README_CN.md` 新增「瓦片地图模式」章节与
   **5 张截图画廊**（地块生态底图、展示地形地图预览、画笔与 16-tile 自动墙、
   编辑瓦片、柏林噪声大地图 + 素材）；截图统一改名归档到 `docs/screenshots/`：
   `01-ecosystem-sheet.png`、`02-map-preview-showcase.png`、`03-map-preview-brushes.png`、
   `04-tile-editor.png`、`05-perlin-world-with-props.png`。
   （v1.0.0 起截图整体重拍，文件名为 `01-solo-pipeline.png` … `07-map-preview-terrains.png`，
   另加素材总览 `showcase-trees.png` / `showcase-npcs.png`；上列旧文件名已不再使用。）
4. 测试：新增 `test_cross_pack_boundary_percolation`（不同包地面色不同时两侧颜色
   互相咬入、仍全不透明、可复现、关闭后恢复原样）；全量测试通过。

## 21. 第十七轮：移除 FrameRonin 图集布局约定

早前（第九轮）搬运进来的 FrameRonin **3×24 blob47 图集布局**与 **16 图块族图集**已整体删除：

- **删除**：`core/tilemap/blob47.py`（掩码→槽位表、3×24 图集构建、16 图块族图集、就近回退、
  槽位切片工具）与其 11 项测试；界面「FrameRonin 47（3×24 布局）」「16 图块族（4×4）」两个模式选项；
  导出目录里的 `atlas/terrain_<id>_blob47.png`；`ui/i18n.py` 对应词条。
- **保留并迁出**：世界生成部分（Perlin + FBM + 河谷 + 山地场）迁到新模块
  `core/tilemap/terrain_noise.py`（`Perlin2D` / `ProceduralTerrain` / `neighbour_mask`），
  去掉对第三方图集槽位的依赖：`sample_tile()` 直接返回**本项目的 canonical 掩码**，
  `grid()` 返回 `(kinds, masks)`；柏林噪声大地图功能不受影响。
- 现在瓦片地图只有两套自研布局：**47-tile 8×6 图集**与**双网格**；
  建筑侧是自研的 **16-tile 墙体族**（与已删除的第三方 16 图集无关）。
## 22. 第十八轮：项目更名为 PixelFoundry IDE（像素铸造 IDE）

| 用途 | 名称 |
|---|---|
| 中文名 | 像素铸造 IDE |
| 英文名 | PixelFoundry IDE |
| 仓库名 | PixelFoundry-IDE（`https://github.com/xf785/PixelFoundry-IDE`） |
| 全称 | PixelFoundry — Pixel Game Asset Foundry |

- **代码**：`config/settings.py` 拆分为四个常量 —— `APP_NAME = "PixelFoundry"`（数据/输出目录）、
  `APP_DISPLAY_NAME = "PixelFoundry IDE"`（窗口标题）、`APP_FULL_NAME`（全称，logo 提示/关于）、
  `APP_NAME_ZH = "像素铸造 IDE"`；打包配置改名 `PixelFoundry.spec`，产物为
  `dist/PixelFoundry/PixelFoundry.exe`；日志名、CLI `prog`、演示模式标题、QSS 注释等 61 个文件统一替换。
- **数据目录迁移**：`app_data_dir()` 在新目录不存在而旧名目录存在时**自动重命名迁移**
  （失败则复制），因此更名不会丢配置与密钥；环境变量 `PIXELFOUNDRY_DATA_DIR` 生效，
  旧的 `PIXELANIMIDE_DATA_DIR` 仍兼容。
- **文档**：README（中英）标题与简介改用全称；ROADMAP（中英）标题更新；
  徽章与链接指向新仓库 slug（GitHub 旧地址自动重定向）。
- **仓库**：通过 API 将 `xf785/PixelAnimIDE-IDE` 改名为 `xf785/PixelFoundry-IDE`，
  本地 remote 同步更新；v0.3.0 Release 的标题与说明文字改为新名
  （其中已上传的二进制仍是旧名构建，历史版本资产文件名保持不变）。
- **测试**：新增 `tests/test_project_naming.py`（6 项）——四个名称常量、窗口标题、
  旧目录迁移、新旧环境变量、新目录已存在时不动旧目录、以及**全仓不得残留旧项目名**的扫描。

> 注意：v0.3.0 的发行 zip（`PixelAnimIDE-v0.3.0-win64.zip`）是更名前构建的，界面与 exe 仍是旧名；
> 需要新名的安装包时，重新打包上传即可（建议作为 v0.4.0 发布）。

## 23. 第十九轮：参考图 i2i、俯视 2.5D 瓦片规划、瓦片包保存文生图底图

### 23.1 瓦片文生图支持参考图（图生图）

- `TilemapParams.reference_image`：瓦片底图生成时把参考图作为 i2i 输入（`image_api.call(..., image=bytes)`），
  与 Solo/IDE 用的是同一条通路；读取失败会明确报错（`WorkflowError`）而不是静默变成纯文生图。
- 界面：瓦片地图页参数区新增「参考图」`ReferenceImageBox`（可拖入/粘贴/清除），
  生成时另存到 `<输出>/tilemap/_reference/reference.png` 并把路径交给工作流。
- 适用场景：沿用参考图的配色/风格生成整套瓦片；或让同一世界观的多套瓦片保持一致。

### 23.2 俯视 2.5D：瓦片规划（顶面层 + 崖壁层）

俯视 2.5D 不需要斜视图，关键是**规划出两套瓦片并各自成套**：

| 层 | 瓦片 | 说明 |
|---|---|---|
| 顶面层 | 已有的 47-tile 族（每地形一套） | 高台与平地共用同一套；高台只是在边界露出崖壁 |
| 崖壁层 | 新增 **16-tile 族**（`core/tilemap/cliff.py`） | 高台**南侧**与低地相接时，画在**低地那一格的上半部**（下半部透明露出地面） |

- **崖壁艺术由地形实测推导**（`cliff_art_from_terrain`）：立面 = 地形纹理压暗 72% + 自下而上加深 +
  3px 垂直条纹（岩层/土层感）+ 顶部亮边 + 底部接触阴影 + 1px 描边（相连侧不描边）→ 与顶面同源配色，
  **不需要额外文生图**，也不会出现风格不搭。
- **16-tile 选型**：按相邻崖壁格求位（N/S/W/E），因此长崖壁逐格连续、两端自动收口（侧壁 2px），
  与建筑墙体族共用同一套位约定与槽位名。
- **模型**：`TileMapModel.height_grid`（0=平地，1..4=高台）+ `paint_height()` + `cliff_cells()`
  （「北邻比自己高」的格子）+ `_render_cliffs()`（在地块层之后、墙层/overlay 之前合成）；
  `to_dict/from_dict` 保留高度层。演示地图会自动抬出一块高原展示南缘崖壁。
- **界面**：参数区「2.5D 高度层」开关（推导崖壁）；预览工具栏新增「高度」微调框 ——
  0 仍是地形画笔，正数抬高、负数降低，可实时看到崖壁生成/消失。

### 23.3 瓦片包保存文生图底图

- `TilePack.sheets`：保存文生图得到的原始网格底图（地块 2×2×3、建筑 2×2、素材变体网格）。
- 导出目录新增 `source/ai_sheet.png`（AI 原图）与 `source/ai_sheet_clean.png`（去格线后的版本），
  `map/info.json` 记录 `source_sheets`，`manifest.json` 记录 `sheets` 清单；`.tilepack` zip 同样包含。
- 导入时会把底图读回 `TilePack.sheets`，因此"再看一眼 AI 原图 / 重新裁切"不需要重新生成。
## 24. 第二十轮：修正「成品比文生图差很多」+ 2.5D 自动高度（水岸/岩石）

### 24.1 画质根因与修正（`core/tilemap/fidelity.py`）

审查 crop→47-tile 链路后确认三处**真问题**（都在"AI 一格 → 瓦片纹理"这一步）：

1. **两次偏移错位缝合**：旧 `make_tile_texture` 先整图 roll 半格 + 十字带交叉淡化，缩放后又缝合一次
   —— 相当于把 AI 画面平移 25% 并淡化掉约 25% 的像素（32 格上 8 列 + 8 行），细节被抹平/重影；
2. **非整数倍最近邻**：`inset` 裁边后 128 → 115，115/32 不是整数倍 → `resize_tile` 退回最近邻，
   像素格色被采丢、出现锯齿；
3. **九格中值之后又缝合 + 量化**：二次损伤。

新实现：**整数倍窗口块众数降采样（一次到位）+ 最佳偏移（与 AI 像素格对齐）+ 1px 边缘焊合**：

- 整格正好是瓦片整数倍时（我们请求的生图尺寸就是 `cells×cell_px`，通常成立）用**整格**、不裁边、
  倍率精确 → AI 按 k 倍画的像素格被逐格还原；
- 窗口偏移强制为 k 的整数倍（否则块众数会跨格取值、颜色错位）；
- 只把最外一圈行列改为两侧边缘均值（左右相等、上下相等）→ 保住"相邻瓦片共享边逐像素相等"，
  其余像素**与 AI 原图逐像素一致**（测试断言改动 ≤ 最外圈像素数、内部不引入源图没有的颜色）；
- 默认不再量化（`max_colors=0`），避免破坏渐变/抖动。

### 24.2 2.5D 自动高度：水岸与岩石

之前只有手绘高度（演示高原）才有崖壁，因此"水岸/岩石处没有 2.5D"。现在按**地形语义自动定高**：

- `_terrain_height_hint(名字)`：含 水/湖/河/海/池/塘/沼泽… → **-1**（洼地，岸上出崖壁）；
  含 岩/石/山/丘/崖/矿/沙丘… → **+1**（高台）；其余 0；
- `TileMapModel.terrain_heights` + `effective_height()`：实际高度 = 地形语义高度 + 手绘高度层；
  `cliff_cells()` 改按有效高度判定 → **水格北侧是陆地时，水格上半部自动出现崖壁**；
- 崖壁**材质取较高一侧**的地形（水岸露出的是岸上的泥土/岩石，而不是水面）；
- `terrain_heights` 随地图 JSON 序列化，预览/导出/大地图都保留；
- 同一生态内不同地形交界仍走块状渗透融合（`edge_blend`），不会出现硬边。

**已知限制**：目前只画**南侧**崖壁（俯视 2.5D 的常规做法）；东西侧壁、斜坡/楼梯、多级高度差
尚未实现；相邻瓦片在"条带 + 边缘噪声"宽度内允许有差异（这是边界有机起伏的来源），
最外圈像素与条带外区域仍严格逐像素一致。
## 25. 第二十一轮：特征块深色框 + 预览 2.5D 无效

1. **特征块瓦片边缘的深色框**：AI 底图在格子四周画了框/描边（或去格线残留 1~3px）时，
   纹理提取会把这一圈当成艺术的一部分 —— 深色被 `measure_edge_profile` 实测成"描边色"，
   于是整族瓦片边缘出现深色框，且与底图观感不符。现在 `faithful_tile_texture()` 先做
   **残留边框检测**：外圈平均亮度低于内部 86% 且方差小（说明是均匀暗线而不是艺术细节）时，
   自动裁掉 1~2px 再提取；测试同时保证"正常艺术不被误裁"。
2. **预览里 2.5D 无效果**：`CliffArt` 是运行时对象、不进地图 JSON，页面预览从 JSON 重建模型后
   没有崖壁艺术 → `_render_cliffs()` 直接返回，高度画笔也没反应。现在：
   - 页面预览在 `model.set_terrain(...)` 之后**重新挂载** `session.cliff_arts` 与 `terrain_heights`；
   - `TilemapView.add_pack()` 加载地块包时自动推导崖壁艺术（并读取包 `meta.terrain_heights`）；
   - 瓦片包 `meta` 新增 `terrain_heights`，跨包加载也能恢复水/岩的高度语义。
   
   测试：`test_preview_model_needs_cliff_arts_rebuilt_from_session`（未挂崖壁 = 平面渲染，
   挂上后水岸出现崖壁）、`test_pack_meta_carries_terrain_heights`。
## 26. 第二十二轮：像素画布的瓦片包/素材包支持、左栏切换器、英文 i18n 补全

### 26.1 像素独立画布：瓦片包 / 素材包导入 + 可视化切换

新增 `ui/widgets/pack_browser.py`（`PackBrowser`）并接入 `ui/pages/pixel_page.py` 左栏
「瓦片包 / 素材包」分组：

- **导入**：`.tilepack`、导出 zip、导出文件夹（统一走 `load_tileset()`）；支持批量恢复；
- **切换器**（清晰便捷的分类可视化）：分段按钮 **全部 / 地形 / 建筑 / 素材 / 底图** +
  **搜索框**（按名字过滤），与资源网格联动；
- **包列表**：带复选框（取消勾选即从资源网格与统计里隐藏）、点选切换当前包、可移除/清空；
- **资源网格**：图标模式 + 最近邻放大缩略图（地形纹理、建筑 16 族拼件、素材、文生图底图），
  悬停显示「包名·资源名」，双击即送入画布；
- **动作**：「放入画布」（居中合成、**保持画布尺寸**）、「替换画布」（尺寸随资源）、「移除包/清空」；
- **持久化**：加载过的包路径写入 `UISettings["pixel_pack_paths"]`，下次进入像素模式自动恢复；
- 主题：`dark.qss` / `light.qss` 新增 `SegmentButton` / `PackList` / `AssetGrid` 样式。

### 26.2 英文 i18n 补全

- 补齐 **51 条**缺失英文（瓦片地图页、2.5D 相关提示、地图预览工具栏、瓦片集导出、Solo/精灵图收尾文案等）；
- 新增 `tests/test_i18n_coverage.py`（4 项）：**全仓中文串 100% 有英文**、
  英文值非空且不含中文、**中英占位符 `{0}` 一致**、语言切换往返；
- 以后新增界面文案若忘记补英文，测试会直接失败。
