...
- **当前进度**：阶段 1（MVP）+ 阶段 2（IDE 模式基础）已完成，见 [开发阶段计划](#开发阶段计划)
...
- [x] **阶段 3（部分）**：APNG/雪碧图/JSON 导出、洋葱皮、调色板锁定、背景去除参数（容差/羽化）暴露到 UI
- [ ] 阶段 3 剩余：多种像素化算法与参数调节、多候选图片选择/局部重绘、大尺寸帧处理性能优化
- [ ] 阶段 4：PyInstaller 打包发布
