...
- **导出 PNG** 存盘。
- **瓦片包 / 素材包**：左栏可导入 `.tilepack`、导出 zip 或导出文件夹，用分段切换器
  （全部 / 地形 / 建筑 / 素材 / 底图）+ 搜索 + 缩略图浏览，再「放入画布」（居中合成、保持画布尺寸）
  或「替换画布」；加载过的包会记住，下次自动进入时恢复。
...
| 独立像素板块 | 第 4 个模式：分辨率设置（**画布设置栏可收起**，画布更大；**分栏可拖拽调宽**）、与 IDE 双向同步、用作图生视频首帧、导出 PNG、**左栏瓦片包/素材包浏览器**（分类切换 + 搜索 + 缩略图 + 放入/替换画布 + 记忆恢复） |
