...
- **瓦片包**：导出**完整瓦片集目录 + zip**（47 图集、FrameRonin 3×24 布局、逐张瓦片、全部元信息、素材/拼件 PNG）；导入支持文件夹、zip 与 `.tilepack`。
- **地图预览**：**无需先生成**即可进入，加载多个包，把地块 / 建筑 / 素材混着铺，网格开关、**Ctrl+左键平移**、滚轮缩放，还能生成**柏林噪声大地图**（最大 400×400，可散布建筑）。
- 生成与预览都支持**双网格**与 **FrameRonin 47/16 布局**。
...
| 独立像素板块 | 第 4 个模式：分辨率设置（**画布设置栏可收起**，画布更大；**分栏可拖拽调宽**）、与 IDE 双向同步、用作图生视频首帧、导出 PNG |
